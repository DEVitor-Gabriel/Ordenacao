...
class Ordena
{
    public:
        ...
        static int procura(Item**, int );
        static int PesquisaBinaria (Item**, int );
        

};

int Ordena::PesquisaBinaria (Item**vet, int tam)
{
     ...
     {
          meio = (inf + sup)/2;
          if (chave == vet[meio]->getChave())
          	   cout<<"\n\nElemento encontrado : "<<vet[meio]->getChave()<<"\n\n";
               return meio;
          if (chave < vet[meio]->getChave())
               sup = meio-1;
          else
     ...
     cout<<"Busca Binaria mal sucedida";
     return -1;   // não encontrado
}

 int Ordena::procura(Item** vetor, int tamanho) {
     int i;
     ...
     for (i = 0; i < tamanho; i++) {
         if (vetor[i]->getChave() == elementoProcurado) {
...
void testaOrdena::menu(){
	...
	switch(op)
	{
		...
		case 5:
			...
			Ordena::PesquisaBinaria(vetorCopia,tam);
